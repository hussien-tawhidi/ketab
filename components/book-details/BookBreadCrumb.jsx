...
export default function BookBreadCrumb({ categories = [], title }) {
  return (
    <div className='border-b border-ketab-light'>
      <nav className='flex flex-wrap items-center gap-2 text-sm px-4 py-2'>
        {/* Home */}
        <Link
          href='/'
        ...
        {categories.map((cat, idx) => (
          <div key={idx} className='flex items-center'>
            <FaChevronLeft className='mx-2 text-xs opacity-40' />
            <Link
              href={`/category/${cat}`}
              className='px-2 py-1 rounded-lg hover:bg-ketab-white/5 transition text-ketab-gray hover:text-ketab-white'>
              {cat}
        ...
        <div className='flex items-center'>
          <FaChevronLeft className='mx-2 text-xs opacity-40' />
          <span className='px-2 py-1 rounded-lg bg-ketab-green/10 text-ketab-green font-semibold'>
            {title}
          </span>
        </div>
      </nav>
