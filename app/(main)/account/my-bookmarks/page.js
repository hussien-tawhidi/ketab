...
import { removeFromBookMarks } from "@/store/slice/bookMarkSlice";
import Image from "next/image";
import { useEffect, useState } from "react";
import { BsBookmark, BsHeart, BsTrash } from "react-icons/bs";
import { useDispatch, useSelector } from "react-redux";

export default function MyBookMarksPage() {
  const [mounted, setMounted] = useState(false);

  const bookmarks = useSelector((item) => item.bookMarks.items);
  ...
  return (
    <div className='max-w-5xl mx-auto p-6 h-full'>
      <h1 className='text-2xl font-bold mb-6 flex items-center gap-2'>
        <BsBookmark className='w-6 h-6 text-ketab-green' />
        لیست نشان‌شده‌ها
      </h1>
              ...
              <div className='p-4 space-y-2'>
                ...
                <div className='flex items-center justify-between mt-3'>
                  ...
                  <div className='flex gap-2'>
                    ...
                    <button
                      onClick={() => dispatch(removeFromBookMarks(item._id))}
